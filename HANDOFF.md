...
**Themes (v2.5):** `skins.js` section (after ART) defines `SKINS.medieval` and `SKINS.scifi`. `applySkin(id)` renames the commodities/powers/groups in place (internal keys `wheat…gold` never change, so saves and multiplayer are unaffected), sets `WORLD`, the page title and `html[data-skin]`. Sci-fi art: `charSVGScifi` (looks: buzz cut + comm, long hair + circlet, pilot helmet + visor, hood + cyber eye), `DIE_STYLES_SCIFI` (cyan community, amber private, violet preview), `ICONS_SCIFI`, hex credit chips, hex badge, recoloured event glyphs, star-field ambience, synth sounds. Wording: `EVENT_TEXT.scifi` rewrites all 37 event cards (same effects); `skinText` swaps currency/world/market words in rendered text only (never inside tags); `goodsSwap` fixes commodity names in cached strings. CSS: tokens redefined under `html[data-skin="scifi"]` plus component overrides (glass HUD panels, corner brackets, holographic grid table, glowing dice). The theme is chosen once when the table is set: pass-and-play setup (stored in `S.skin`, remembered per device as `mt-skin`) or the host's lobby (action `skin`, host only, lobby phase only; every phone follows). It is locked once play starts; there is no in-game toggle. The title screen shows the device's last theme. Fonts Orbitron + Chakra Petch are self-hosted.

**Theme consistency (v2.5.1):** every rendered surface (screens, sheets, bar, dice overlay, connection banner, toasts, share sheet) goes through `skinText`; user-typed chat and player names are never altered. In sci-fi, "goods" reads "cargo"; the medieval card "Credit Crunch" is now "Moneylenders' Squeeze" (so "credit" never appears in medieval) and "Market Boom" no longer says "exchange". Toasts clear when the theme changes. The harness scans every event card, effect, tip and host log line in both themes for the other theme's words; a browser crawl (not in the repo) checked ~15,000 screen states across full solo and multiplayer games, theme switches between games, resumes and guests leaving, with zero leaks.

**Stability (v2.4):**
- *Rendering:* `patchDOM` diffs new markup into the page with morphdom (inlined, MIT) instead of replacing `innerHTML`, so taps and incoming updates no longer replay entrance animations (the "flash"). Screens, sheets and the betting tray carry ids (`scr-<screen>`, `sh-<type>`, `tray-<hand>-<street>-<turn>`) so animations play only when those actually change.
- *Transport:* data connections use JSON serialization (PeerJS's default binary chunking is unreliable on mobile Safari). ICE servers: Google + Cloudflare STUN, PeerJS TURN (UDP 3478) and Open Relay TURN on 80/443 incl. TLS-over-TCP for networks that block UDP. Views cap private messages at the last 150 per seat (typical view ≈ 6–30 KB).
- *Recovery:* guests retry stalled connects after 12 s, rebuild a destroyed peer, show a "Waiting for <host>'s phone…" banner after 9 s of silence and reconnect after 25 s; the host re-creates its peer if the signaling link stays down, and marks a seat Away after 25 s. `mt-active` remembers the table: reloading (or a tab discard) auto-rejoins as guest or auto-resumes hosting; tapping Leave clears it. While a guest is disconnected, action buttons are locked (`body.net-down`).
- *Tested:* a four-session chaos test (guest page close/reopen, guest reload, abrupt connection cut, signaling-server restart, host reload) — 29 disruptions across 3 randomized full games, every one recovered automatically in ≤ 3 s with live connections, delivered messages and identical state on all phones.

**Trading (v2.3):** one trade per market phase, now an *order* mixing buys and sells (`tradeQuote/doTrade` mode `"order"` with `buy:{k:q}` and `sell:{k:q}`). Rules: sale proceeds fund purchases; can't buy and sell the same good in one order; buying is free; selling 2+ kinds pays the sell fee on sales (10% if the merchant starts at 0 florins); resulting florins must be ≥ 0. Old modes (`buy`, `sell`, `multi`) remain for compatibility. UI: one signed stepper per commodity (− sells, + buys) with a live summary; "Execute trade" commits it. **Trade prompt:** `maybeTradePrompt` opens `shTradePrompt` once per hand when the market opens (after any event choice): in multiplayer, on each phone that hasn't traded or readied; in pass-and-play, one list of every merchant. Tips come from `tradeHints(p)` (engine, tested): 1–2 non-prescriptive suggestions (short of the ante, thin purse, last hand, a held good near its peak, a two-hand climber, cheap goods when flush, short of a power's cost). Per-phone setting `mt-hints` (default on); off = no prompt. Toggles in setup, the lobby and on the prompt.

**Private messages (v2.2):** 1:1 only, no group threads. Action `{type:"dm", to:<seat number>, text}` → `hostDM` validates the recipient, strips control characters, caps text at `DM_MAX` (300), rate-limits to 8 per 5 s per seat, stamps sender/time/id, and keeps the last 500 in `S.dms`. `makeView` includes only messages where the viewer is sender or recipient, so a third phone never receives them. Client: `renderChat` draws a floating bubble with unread badge, a contact list (online dot, last message, unread count), and a conversation with grouped bubbles, timestamps, quick replies and a composer. The chat lives in its own `#chat` root and is patched in place, so typing isn't interrupted by game updates. Read markers are per phone in localStorage (`mt-chat-read-CODE`). Pass-and-play has no chat.
